...
<section class="panel filter-panel security-filter-panel">
    <form method="get" action="<?= site_url('distribusi-dokumen') ?>" class="distribution-filter-form">
        ...
        <?= view('components/list_order_filter', ['id' => 'distribusi_urutan', 'value' => $filters['order']]) ?>
...
<?php if ($filters['tab'] === 'masuk'): ?>
    <section class="panel register-panel distribution-table-panel security-register-panel">
        ...
        <div class="table-list-footer">
            <form method="get" action="<?= site_url('distribusi-dokumen') ?>" class="table-length-form"><input type="hidden" name="tab" value="masuk"><input type="hidden" name="q" value="<?= esc($filters['keyword']) ?>"><input type="hidden" name="dari" value="<?= esc($filters['from']) ?>"><input type="hidden" name="sampai" value="<?= esc($filters['to']) ?>"><input type="hidden" name="urutan" value="<?= esc($filters['order']) ?>"><label for="distribusi_per_page">Tampilkan</label><select id="distribusi_per_page" name="per_page" aria-label="Jumlah baris per halaman" data-table-length><?php foreach ([10, 20, 50, 100] as $size): ?><option value="<?= $size ?>" <?= $filters['perPage'] === $size ? 'selected' : '' ?>><?= $size ?></option><?php endforeach ?></select><span>data</span></form><?php if ($dokumen !== []): ?><div class="pagination-wrap"><?= $pager->links('distribusi_dokumen', 'default_full') ?></div><?php endif ?>
        </div>
    </section>
<?php else: ?>
    <section class="panel register-panel distribution-table-panel distribution-outgoing-panel security-register-panel">
        <div class="table-wrap">
            <table>
                ...
                <tbody>
                    <?php if ($dokumenKeluar === []): ?><tr>
                            <td colspan="9">
                                <div class="empty-state"><span>⇢</span><strong>Belum ada Dokumen Keluar untuk diproses</strong>
                                    <p>Dokumen dari Progres Dokumen Agendaris akan muncul otomatis di bagian ini.</p>
                                </div>
                            </td>
                        </tr>
                    <?php else: ?>
        ...
        <div class="table-list-footer">
            <form method="get" action="<?= site_url('distribusi-dokumen') ?>" class="table-length-form"><input type="hidden" name="tab" value="keluar"><input type="hidden" name="q" value="<?= esc($filters['keyword']) ?>"><input type="hidden" name="dari" value="<?= esc($filters['from']) ?>"><input type="hidden" name="sampai" value="<?= esc($filters['to']) ?>"><input type="hidden" name="urutan" value="<?= esc($filters['order']) ?>"><label for="distribusi_keluar_per_page">Tampilkan</label><select id="distribusi_keluar_per_page" name="per_page" aria-label="Jumlah Surat Keluar per halaman" data-table-length><?php foreach ([10, 20, 50, 100] as $size): ?><option value="<?= $size ?>" <?= $filters['perPage'] === $size ? 'selected' : '' ?>><?= $size ?></option><?php endforeach ?></select><span>data</span></form><?php if ($dokumenKeluar !== []): ?><div class="pagination-wrap"><?= $pagerKeluar->links('distribusi_keluar', 'default_full') ?></div><?php endif ?>
        </div>
    </section>
<?php endif ?>

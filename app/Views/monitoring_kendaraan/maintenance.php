...
<section class="page-heading heading-actions vehicle-page-heading">
    <div>
        ...
        <h1>Servis dan Perawatan</h1>
        <p>Catat pekerjaan bengkel, biaya, kilometer, dan jadwal servis berikutnya.</p>
    </div>
...
<section class="panel filter-panel vehicle-filter-panel">
    <form method="get" class="vehicle-filter-form vehicle-filter-simple">
    ...
    <div class="table-wrap">
        <table>
            ...
            <tbody>
                <?php if ($records === []): ?><tr>
                        <td colspan="9">
                            <div class="empty-state"><span>⚙</span><strong>Belum ada data servis</strong>
                                <p>Catatan servis dan perawatan kendaraan akan tampil di halaman ini.</p>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr class="<?= $isDeleted ? 'vehicle-row-deleted' : '' ?>">
                        <td><strong><?= (($pager->getCurrentPage('maintenance') - 1) * $filters['perPage']) + $index + 1 ?></strong></td>
                        <td>
                            <div class="vehicle-name-cell"><strong><?= esc($record['nomor_polisi']) ?></strong><span><?= esc($vehicleDisplayName) ?></span><?php if ($isDeleted): ?><small class="vehicle-deleted-note"><?= ! empty($record['deleted_at']) ? 'Data servis dihapus' : 'Kendaraan dihapus' ?></small><?php endif ?></div>
                        </td>
                        <td><?= date('d-m-Y', strtotime($record['tanggal_servis'])) ?></td>
                        <td><strong><?= esc($record['jenis_perawatan']) ?></strong></td>
                        <td><?= esc($record['bengkel'] ?: '-') ?></td>
                        <td><?= $record['kilometer'] !== null ? number_format((int)$record['kilometer'], 0, ',', '.') . ' km' : '-' ?></td>
...
<div class="account-modal vehicle-crud-modal" id="maintenanceDetailModal" hidden aria-hidden="true"><button type="button" class="modal-backdrop" data-maintenance-detail-close aria-label="Tutup detail"></button>
    <section class="modal-dialog vehicle-crud-dialog vehicle-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="maintenanceDetailTitle">
        <header class="modal-header">
            <div class="modal-title-group"><span class="modal-title-icon">⌕</span>
                <div>
                    <p>MONITORING KENDARAAN</p>
                    <h2 id="maintenanceDetailTitle">Detail Servis</h2>
                </div>
            </div><button type="button" class="modal-close" data-maintenance-detail-close>×</button>
        </header>
        <div class="modal-body vehicle-detail-body">
            <section class="vehicle-detail-section">
                <header><span>01</span>
                    <div><strong>Jadwal Servis</strong><small>Kendaraan dan tanggal pelaksanaan</small></div>
                </header>
                <dl class="modal-detail-grid">
                    <div class="detail-wide">
                        <dt>Kendaraan</dt>
                        <dd data-maintenance-detail-field="kendaraan">-</dd>
                    </div>
                    <div>
                        <dt>Tanggal Servis</dt>
                        <dd data-maintenance-detail-field="tanggal_servis">-</dd>
                    </div>
                    <div>
                        <dt>Servis Berikutnya</dt>
                        <dd data-maintenance-detail-field="servis_berikutnya">-</dd>
                    </div>
                </dl>
            </section>
            <section class="vehicle-detail-section">
                <header><span>02</span>
                    <div><strong>Detail Perawatan</strong><small>Pekerjaan, bengkel, kilometer, dan biaya</small></div>
                </header>
                <dl class="modal-detail-grid">
                    <div>
                        <dt>Jenis Perawatan</dt>
                        <dd data-maintenance-detail-field="jenis_perawatan">-</dd>
                    </div>
                    <div>
                        <dt>Bengkel</dt>
                        <dd data-maintenance-detail-field="bengkel">-</dd>
                    </div>
                    <div>
                        <dt>KM Saat Servis</dt>
                        <dd data-maintenance-detail-field="kilometer">-</dd>
                    </div>
                    <div>
                        <dt>Biaya</dt>
                        <dd data-maintenance-detail-field="biaya">-</dd>
                    </div>
                </dl>
            </section>
            <section class="vehicle-detail-section">
                <header><span>03</span>
                    <div><strong>Anggaran &amp; Dokumen</strong><small>Sumber anggaran dan informasi pendukung</small></div>
                </header>
                <dl class="modal-detail-grid">
                    <div>
                        <dt>Anggaran Service</dt>
                        <dd data-maintenance-detail-field="anggaran_servis">-</dd>
                    </div>
                    <div>
                        <dt>Nama Perusahaan</dt>
                        <dd data-maintenance-detail-field="nama_perusahaan">-</dd>
                    </div>
                    <div class="detail-wide">
                        <dt>Link Berkas</dt>
                        <dd><a data-maintenance-detail-link target="_blank" rel="noopener noreferrer" hidden>Buka berkas ↗</a><span data-maintenance-detail-no-link>-</span></dd>
                    </div>
                    <div class="detail-wide">
                        <dt>Keterangan</dt>
                        <dd data-maintenance-detail-field="keterangan">-</dd>
                    </div>
                </dl>
            </section>
        </div>
        <footer class="modal-footer"><button type="button" class="btn btn-secondary" data-maintenance-detail-close>Tutup</button></footer>
    </section>
</div>
...
<div class="account-modal vehicle-crud-modal" id="vehicleCrudFormModal" hidden aria-hidden="true"><button type="button" class="modal-backdrop" data-vehicle-crud-form-close></button>
    <section class="modal-dialog vehicle-crud-dialog" role="dialog" aria-modal="true" aria-labelledby="maintenanceFormTitle">
        <header class="modal-header">
            <div class="modal-title-group"><span class="modal-title-icon" data-crud-form-icon>＋</span>
                <div>
                    <p>MONITORING KENDARAAN</p>
                    <h2 id="maintenanceFormTitle" data-crud-form-title>Tambah Servis</h2>
                </div>
            </div><button type="button" class="modal-close" data-vehicle-crud-form-close>×</button>
        </header>
        <form method="post" action="<?= site_url('bagian-umum-2/monitoring-kendaraan/servis-perawatan') ?>" data-vehicle-crud-form><?= csrf_field() ?><div class="modal-body vehicle-wizard-body">
                <ol class="vehicle-form-stepper maintenance-form-stepper" aria-label="Tahapan form servis">
                    <li class="active" data-maintenance-step-indicator="0"><span>01</span>
                        <div><strong>Informasi Servis</strong><small>Kendaraan, jadwal, dan perawatan</small></div>
                    </li>
                    <li data-maintenance-step-indicator="1"><span>02</span>
                        <div><strong>Biaya &amp; Dokumen</strong><small>Kilometer, anggaran, dan berkas</small></div>
                    </li>
                </ol>
                <section class="modal-form-grid vehicle-form-grid vehicle-form-step" data-maintenance-form-step="0">
                    <div class="form-group modal-span-2"><label for="maintenanceVehicle">Kendaraan <span>*</span></label><select id="maintenanceVehicle" name="vehicle_id" required>
                    ...
                    <div class="form-group"><label for="maintenanceDate">Tanggal Servis <span>*</span></label><input id="maintenanceDate" name="tanggal_servis" type="date" value="<?= date('Y-m-d') ?>" required><small data-service-date-lock hidden>Tanggal akan tersimpan secara otomatis.</small></div>
                    <div class="form-group"><label for="maintenanceNextDate">Servis Berikutnya</label><input id="maintenanceNextDate" name="servis_berikutnya_tanggal" type="date" readonly><small>Terisi otomatis setelah Tanggal Servis diinput.</small></div>
                    <div class="form-group"><label for="maintenanceType">Jenis Perawatan <span>*</span></label><input id="maintenanceType" name="jenis_perawatan" maxlength="150" placeholder="Servis berkala / ganti oli" required></div>
                    <div class="form-group"><label for="maintenanceWorkshop">Bengkel</label><input id="maintenanceWorkshop" name="bengkel" maxlength="150"></div>
                </section>
                <section class="modal-form-grid vehicle-form-grid vehicle-form-step" data-maintenance-form-step="1" hidden>
                    <div class="form-group"><label for="maintenanceKm">KM Saat Servis <span>*</span></label><input id="maintenanceKm" name="kilometer" type="number" min="0" required></div>
                    <div class="form-group"><label for="maintenanceCost">Biaya <span>*</span></label><input id="maintenanceCost" name="biaya" type="number" min="0" step="1" value="0" required></div>
                    <div class="form-group"><label for="maintenanceBudget">Anggaran Service <span>*</span></label><select id="maintenanceBudget" name="anggaran_servis" required>
                            <option value="">Pilih anggaran service</option><?php foreach ($serviceBudgets as $budget): ?><option value="<?= esc($budget) ?>"><?= esc($budget) ?></option><?php endforeach ?>
                        </select></div>
...
<script>
    (() => {
        ...
        const synchronizeNextDate = () => {
            nextDate.value = calculateNextDate(serviceDate.value);
        };
        ...
        const showStep = index => {
            ...
            steps[currentStep].querySelector('input:not([disabled]), select:not([disabled]), textarea:not([disabled])')?.focus({
                preventScroll: true
            });
        };

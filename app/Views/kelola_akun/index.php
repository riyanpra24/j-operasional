        ...
        <table>
            ...
            <tbody>
                <?php if ($users === []): ?>
                    <tr>
                        <td colspan="6">
                            <div class="empty-state compact"><span>♙</span><strong>Akun tidak ditemukan</strong>
                                <p>Ubah filter atau tambahkan akun baru.</p>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($users as $index => $user): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="account-name-cell"><span><?= esc(strtoupper(substr($user['display_name'], 0, 1))) ?></span><strong><?= esc($user['display_name']) ?></strong></div>
                            </td>
...
<div class="account-modal" id="accountFormModal" hidden aria-hidden="true">
    ...
    <section class="modal-dialog account-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="accountFormTitle">
        <header class="modal-header">
            <div class="modal-title-group"><span class="modal-title-icon" data-account-form-icon>＋</span>
                <div>
                    <p>ADMINISTRATOR</p>
                    <h2 id="accountFormTitle" data-account-form-title>Tambah Akun</h2>
                </div>
            </div><button type="button" class="modal-close" data-account-modal-close aria-label="Tutup">×</button>
        </header>
        <form action="<?= site_url('kelola-akun') ?>" method="post" data-account-form>
            <?= csrf_field() ?>
            <div class="modal-body">
                <div class="modal-section-heading"><span>01</span>
                    <div><strong>Data akun</strong><small>Lengkapi identitas dan hak akses pengguna</small></div>
                </div>
                <div class="modal-form-grid">
                    ...
                    <div class="modal-span-2 admin-credential-verification" data-admin-credential-verification hidden>
                        <div class="modal-section-heading"><span>02</span>
                            <div><strong>Verifikasi perubahan</strong><small>Wajib diisi ketika password atau PIN admin akan diganti</small></div>
                        </div>
                        <div class="modal-form-grid">
    ...
    <section class="modal-dialog account-modal-dialog" role="dialog" aria-modal="true" aria-labelledby="accountDetailTitle">
        <header class="modal-header">
            <div class="modal-title-group"><span class="modal-title-icon">♙</span>
                <div>
                    <p>DETAIL AKUN</p>
                    <h2 id="accountDetailTitle">Informasi Pengguna</h2>
                </div>
            </div><button type="button" class="modal-close" data-account-detail-close aria-label="Tutup">×</button>
        </header>
        <div class="modal-body">
            <dl class="modal-detail-grid">
                <div>
                    <dt>Nama pengguna</dt>
                    <dd data-account-detail="display_name">-</dd>
                </div>
                <div>
                    <dt>Username</dt>
                    <dd data-account-detail="username">-</dd>
                </div>
                <div>
                    <dt>Role</dt>
                    <dd data-account-detail="role_label">-</dd>
                </div>
                <div>
                    <dt>PIN login admin</dt>
                    <dd data-account-detail="admin_pin_status">-</dd>
                </div>
                <div>
                    <dt>Dibuat</dt>
                    <dd data-account-detail="created_at">-</dd>
                </div>
                <div>
                    <dt>Terakhir diperbarui</dt>
                    <dd data-account-detail="updated_at">-</dd>
                </div>
            </dl>
        </div>
...
<div class="account-modal" id="accountDeleteModal" hidden aria-hidden="true">
    ...
    <section class="modal-dialog delete-modal-dialog" role="alertdialog" aria-modal="true" aria-labelledby="accountDeleteTitle">
        <div class="delete-modal-body"><span class="delete-warning-icon">!</span>
            <h2 id="accountDeleteTitle">Hapus Akun?</h2>
            <p>Akun <strong data-account-delete-name></strong> akan dihapus permanen dan tidak dapat dipulihkan.</p>
        </div>
...
<script>
    (() => {
        ...
        const closeModal = modal => {
            ...
            setTimeout(() => {
                modal.hidden = true;
                if (!document.querySelector('.account-modal.open')) document.body.classList.remove('modal-open');
            }, 180);
        };
        ...
        document.querySelectorAll('[data-account-delete]').forEach(button => button.addEventListener('click', () => {
            const data = JSON.parse(button.dataset.accountDelete);
            deleteModal.querySelector('[data-account-delete-name]').textContent = `${data.display_name} (${data.username})`;
            deleteModal.querySelector('[data-account-delete-form]').action = `${baseUrl}/${data.id}/hapus`;
            openModal(deleteModal);
        }));
        document.querySelectorAll('[data-account-detail-url]').forEach(button => button.addEventListener('click', async () => {
            button.disabled = true;
            try {
                const response = await fetch(button.dataset.accountDetailUrl, {
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                });
                const result = await response.json();
                if (!response.ok || !result.success) throw new Error(result.message || 'Detail akun gagal dimuat.');
                Object.entries(result.user).forEach(([key, value]) => {
                    const target = detailModal.querySelector(`[data-account-detail="${key}"]`);
        ...
        if (failedModal === 'edit' && failedEditId) prepareEdit({
            ...failedData,
            id: failedEditId
        });
    })();

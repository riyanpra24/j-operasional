...
<section class="page-heading heading-actions">
    <div><p class="eyebrow"><?= $title === 'Dokumen Keluar' ? 'SECURITY' : 'AGENDARIS' ?></p><h1><?= esc($title) ?></h1><p>Kelola pengiriman dan penerimaan dokumen keluar.</p></div>
...
<section class="panel filter-panel">
    <form method="get" action="<?= $indexUrl ?>" class="agendaris-filter-form">
        ...
        <div class="form-group"><label for="keluar_jenis_filter">Jenis Surat</label><select id="keluar_jenis_filter" name="jenis"><option value="">Semua jenis</option><?php foreach ($jenisOptions as $option): ?><option value="<?= esc($option) ?>" <?= $filters['jenis'] === $option ? 'selected' : '' ?>><?= esc($option) ?></option><?php endforeach ?></select></div>
...
<section class="panel register-panel agendaris-table-panel">
    <div class="table-wrap"><table><thead><tr><th>No.</th><th>Nomor Surat</th><th>Jenis Surat</th><th>Pemohon</th><th>Pelaksana</th><th>UP</th><th>Tanggal Pengiriman</th><th>Alamat Penerima</th><th>Aksi</th></tr></thead><tbody>
        <?php if ($dokumen === []): ?>
            <tr><td colspan="9"><div class="empty-state"><span>⇢</span><strong>Belum ada Surat Keluar</strong><p>Gunakan tombol Tambah Surat Keluar untuk mencatat pengiriman baru.</p></div></td></tr>
        <?php else: ?>

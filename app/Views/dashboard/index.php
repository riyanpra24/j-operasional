...
/** @var array{total: int, today: int, month: int, items: int, senders: int} $stats */
/** @var list<array{label: string, total: int|string}> $chart */
/** @var list<array{label: string, total: int|string}> $jenis */
/** @var list<array{label: string, total: int|string}> $ekspedisi */
...
<section class="reference-top-grid">
    ...
    <article class="company-summary-card">
        <header>
            <div>
                <h1>Data Dokumen Masuk</h1>
                <p>Ringkasan register dokumen operasional</p>
            </div><button type="button" data-print>Download Report <span>↧</span></button>
        </header>
...
<section class="reference-chart-grid">
    <article class="reference-panel main-graph-card">
        <header>
            <div>
                <h2>Grafik Dokumen Masuk</h2>
                <p>Pergerakan data enam bulan terakhir</p>
            </div><a href="<?= $canAccessSecurity ? site_url('distribusi-dokumen') : $registerUrl ?>" <?= $canAccessSecurity ? 'data-open-input-modal' : '' ?>>＋ <?= $canAccessSecurity ? 'Dokumen Baru' : 'Buka Agenda' ?></a>
        </header>
            ...
            <div class="reference-svg-wrap">
                <svg viewBox="0 0 <?= $chartWidth ?> <?= $chartHeight ?>" preserveAspectRatio="none" aria-label="Grafik dokumen masuk">
                    ...
                    <?php foreach ([40, 70, 100, 130, 165] as $y): ?>
                        <line x1="0" y1="<?= $y ?>" x2="680" y2="<?= $y ?>" class="reference-grid-line" /><?php endforeach ?>
                    ...
                    <?php foreach ($points as $i => $point): ?>
                        <circle cx="<?= $point[0] ?>" cy="<?= $point[1] ?>" r="5" class="reference-chart-point" /><text x="<?= $point[0] ?>" y="<?= max(12, $point[1] - 12) ?>" text-anchor="middle" class="reference-point-label"><?= $chart[$i]['total'] ?></text><?php endforeach ?>
                </svg>
    ...
    <article class="reference-panel donut-project-card">
        <header>
            <div>
                <h2>Jenis Dokumen</h2>
                <p>Komposisi register aktif</p>
            </div><span>⚙</span>
        </header>
        <div class="reference-donut" style="<?= esc($donutStyle, 'attr') ?>">
            <div><strong><?= number_format($stats['total'], 0, ',', '.') ?></strong><small>Total</small></div>
        </div>
        <div class="reference-donut-legend">
            <?php if ($jenisTop === []): ?><span><i class="gray"></i>Belum ada data</span><?php else: ?>
                <?php $legendClasses = ['orange', 'green', 'blue'];
                                                                                            foreach ($jenisTop as $i => $item): ?><span><i class="<?= $legendClasses[$i] ?>"></i><?= esc($item['label']) ?></span><?php endforeach ?>
            <?php endif ?>
        </div>
    </article>
</section>
...
<section class="reference-data-section">
    <header class="reference-section-header">
        <div>
            <h2>Data Grafik</h2>
            <p>Analitik singkat dokumen masuk</p>
        </div>
        <div class="reference-search">⌕&nbsp; Cari pada tabel dokumen</div><a href="<?= $registerUrl ?>">Lihat Semua <span>→</span></a>
    </header>
    <div class="reference-data-grid">
        <article class="ring-stat-card">
            <div class="reference-ring orange-ring" style="--ring-value:<?= $todayRate * 3.6 ?>deg"><span><?= $todayRate ?>%</span></div>
            <div><span>Dokumen Hari Ini</span><strong><?= number_format($stats['today'], 0, ',', '.') ?></strong><small>Masuk pada <?= date('d-m-Y') ?></small></div>
        </article>
        <article class="ring-stat-card">
            <div class="reference-ring green-ring" style="--ring-value:<?= $monthRate * 3.6 ?>deg"><span><?= $monthRate ?>%</span></div>
            <div><span>Dokumen Bulan Ini</span><strong><?= number_format($stats['month'], 0, ',', '.') ?></strong><small>Dari <?= number_format($stats['senders'], 0, ',', '.') ?> pengirim</small></div>
        </article>
        <article class="mini-analytics-card">
            <div><strong><?= number_format($stats['items'], 0, ',', '.') ?></strong><span>Total Jumlah Dokumen</span></div><svg viewBox="0 0 300 70" preserveAspectRatio="none">
                <polyline points="<?= esc(implode(' ', array_map(static fn($p) => round(($p[0] / 680) * 300, 1) . ',' . round(max(5, $p[1] / 2.7), 1), $points))) ?>" />
            </svg>
        </article>
    </div>
</section>
...
<section class="reference-panel reference-recent-table">
    <header>
        <div>
            <h2>Dokumen Masuk Terbaru</h2>
            <p>Data yang terakhir dicatat</p>
        </div><a href="<?= $registerUrl ?>">Buka Register →</a>
    </header>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Perihal</th>
                    <th>Penerima</th>
                    <th>Hari / Tanggal Diterima</th>
                    <th>Jenis</th>
                    <th>Jumlah</th>
                    <th>Ekspedisi</th><?php if ($canAccessSecurity): ?><th></th><?php endif ?>
                </tr>
            </thead>
            <tbody>
                <?php if ($recent === []): ?><tr>
                        <td colspan="<?= $canAccessSecurity ? 9 : 8 ?>">
                            <div class="empty-state compact"><span>▤</span><strong>Belum ada dokumen masuk</strong>
                                <p>Belum ada data dokumen yang dapat ditampilkan.</p>
                            </div>
                        </td>
                    </tr>
                    <?php else: ?><?php foreach ($recent as $index => $row): ?><tr>
                        ...
                        <td><?= esc($row['ekspedisi'] ?: '-') ?></td><?php if ($canAccessSecurity): ?><td><a class="icon-btn" href="<?= site_url('dokumen-masuk/' . $row['id']) ?>" data-open-detail-modal data-detail-url="<?= site_url('dokumen-masuk/' . $row['id']) ?>">→</a></td><?php endif ?>

...
<section class="panel filter-panel spk-filter-panel">
    <form action="<?= $baseUrl ?>" method="get" class="spk-filter-form">
        ...
        <div class="form-group">
            ...
            <select id="spkCompleteness" name="kelengkapan">
                <option value="">Semua kondisi</option>
                <option value="lengkap" <?= $filters['completeness'] === 'lengkap' ? 'selected' : '' ?>>Lengkap</option>
                <option value="belum_lengkap" <?= $filters['completeness'] === 'belum_lengkap' ? 'selected' : '' ?>>Belum Lengkap</option>
            </select>
        </div>
...
<section class="panel register-panel spk-table-panel">
    <div class="panel-header spk-register-header">
        <div>
            <h2>Register Dokumen<?= $filters['year'] ? ' Tahun ' . $filters['year'] : '' ?></h2>
            <p>Dokumen dengan tanggal atau link kosong otomatis ditandai belum lengkap.</p>
        </div>
        ...
        <table class="spk-register-table">
            ...
            <tbody>
                <?php if ($records === []): ?>
                    <tr>
                        <td colspan="7">
                            <div class="empty-state"><span>▤</span><strong>Dokumen tidak ditemukan</strong>
                                <p>Ubah filter atau tambahkan dokumen baru.</p><button class="btn btn-primary btn-sm" type="button" data-spk-create data-spk-create-year="<?= (int) ($filters['year'] ?: date('Y')) ?>">Tambah Dokumen</button>
                            </div>
                        </td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($records as $index => $record): ?>
                        <tr>
                            ...
                            <td>
                                <div class="spk-number-cell"><button type="button" class="spk-number-trigger" data-spk-view="<?= (int) $record['id'] ?>"><?= esc($record['nomor_dokumen']) ?></button><small>Tahun <?= (int) $record['tahun'] ?></small></div>
                            </td>
                            <td>
                                <div class="date-cell"><strong><?= $formatDate($record['tanggal_dokumen']) ?></strong><span><?= $record['tanggal_dokumen'] ? date('l', strtotime($record['tanggal_dokumen'])) : 'Perlu dilengkapi' ?></span></div>
                            </td>
                            <td>
                                <p class="spk-subject-cell"><?= esc($record['perihal']) ?></p>
                            </td>
                            ...
                            <td>
                                <div class="action-buttons"><button class="icon-btn" type="button" data-spk-view="<?= (int) $record['id'] ?>" title="Lihat detail">⌕</button><button class="icon-btn" type="button" data-spk-edit="<?= (int) $record['id'] ?>" title="Ubah dokumen">✎</button></div>
                            </td>
                        </tr>
                    <?php endforeach ?>
                <?php endif ?>
...
<div class="input-modal spk-edit-modal" id="spkEditModal" hidden aria-hidden="true">
    ...
    <section class="modal-dialog spk-edit-dialog" role="dialog" aria-modal="true" aria-labelledby="spkEditTitle">
        <header class="modal-header">
            <div class="modal-title-group"><span class="modal-title-icon" data-spk-form-icon>✎</span>
                <div>
                    <p data-spk-form-eyebrow>UBAH DOKUMEN SPK</p>
                    <h2 id="spkEditTitle" data-spk-form-title>Edit Data SPK</h2>
                </div>
            </div>
        ...
        <form method="post" action="" data-spk-edit-form>
            ...
            <div class="modal-body spk-edit-body">
                <div class="modal-form-grid">
                    ...
                    <div class="form-group modal-span-2"><label for="spkEditNumber">Nomor SPK <span class="required">*</span></label>
                        <div class="spk-number-generator"><input id="spkEditNumber" name="nomor_dokumen" readonly required><button class="btn btn-secondary" type="button" data-spk-edit-generate>Generate Nomor SPK</button></div><small data-spk-edit-number-hint>Nomor urut dan bulan pembuatan nomor dikelola otomatis oleh sistem.</small>
                    </div>
    ...
    <section class="modal-dialog spk-view-dialog" role="dialog" aria-modal="true" aria-labelledby="spkDetailTitle">
        <header class="modal-header">
            <div class="modal-title-group"><span class="modal-title-icon">▤</span>
                <div>
                    <p>DETAIL DOKUMEN SPK</p>
                    <h2 id="spkDetailTitle" data-spk-detail="nomor_dokumen">-</h2>
                </div>
            </div>
        ...
        <div class="modal-body spk-view-body">
            ...
            <dl class="spk-detail-grid spk-popup-grid">
                <div>
                    <dt>Nomor Urut</dt>
                    <dd data-spk-detail="nomor_urut">-</dd>
                </div>
                <div>
                    <dt>Tahun Register</dt>
                    <dd data-spk-detail="tahun">-</dd>
                </div>
                <div>
                    <dt>Tanggal Dokumen</dt>
                    <dd data-spk-detail="tanggal">-</dd>
                </div>
                <div>
                    <dt>Dibuat Oleh</dt>
                    <dd data-spk-detail="dibuat_oleh">-</dd>
                </div>
                <div class="wide">
                    <dt>Perihal</dt>
                    <dd data-spk-detail="perihal">-</dd>
                </div>
                <div class="wide">
                    <dt>Terakhir Diperbarui</dt>
                    <dd data-spk-detail="diperbarui">-</dd>
                </div>
            </dl>
        </div>
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                const result = contentType.includes('application/json') ? await response.json() : {
                    success: false,
                    message: 'Respons server tidak valid. Muat ulang halaman dan coba kembali.'
                };
                updateCsrf(result);
